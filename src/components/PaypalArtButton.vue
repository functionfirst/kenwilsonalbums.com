<template>
  <div class="flex flex-col self-start rounded bg-base mt-4 p-4 shadow-inner">
    <h2 class="leading-normal font-normal mb-4">Select a size and print</h2>

    <on-click-outside :do="handleClickOutside">
    <div class="relative mb-4" @click="open = !open ">
      <div v-html="selectedText" class="bg-lighter cursor-pointer hover:border-darker p-3 rounded border border-dark shadow-inner"></div>
...
export default {
  ...
  methods: {
    ...
    select(index) {
      ...
      this.selectedText = `${this.options[index].size} <small>(${this.options[index].dimensions})</small>`
    },
    formatPrice(value) {
