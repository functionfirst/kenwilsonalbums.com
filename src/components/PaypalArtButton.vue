<template>
  <div class="flex flex-col self-start rounded bg-base mt-4 p-4 shadow-inner">
    <h2 class="leading-normal text-lg font-normal mb-2">Select a product to order:</h2>
    <PosterDropdown :posters="posters" :types="types" @change="changeProduct" />

    <template v-if="price">
      <h2 class="leading-normal text-lg font-normal mb-1">Delivery:</h2>

      <div class="bg-white rounded shadow-md mb-4">
        <label
          v-for="(post, index) in postageOptions"
          :key="index"
          :class="[shippingSelected === index ? 'font-bold' :  '']"
          class="p-3 cursor-pointer border-b border-base hover:bg-light flex justify-between"
        >
          <input
            type="radio"
            :checked="shippingSelected === index"
            name="delivery"
            @change="updateShipping(post.price, index)"
          />
          <div class="flex-1 ml-3">{{ post.title }}</div>
          <div>
            <span class="ml-2">{{ formatPrice(post.price) }}</span>
          </div>
        </label>
      </div>
    </template>

    <p v-if="error" class="my-1 text-sm text-danger font-bold">{{ error }}</p>

    <div class="flex my-2 justify-between border-t border-dark py-2">
      <span class="font-thin text-lg">Total Price:</span>

      <span class="font-bold text-lg text-right">
        <template v-if="price">
          {{ formatPrice(totalPrice) }}
          <span v-if="!shipping">*</span>
        </template>
        <template v-else>n/a</template>
      </span>
    </div>

    <div class="font-thin text-sm text-right" v-if="price && !shipping">*Not including delivery</div>

    <button
      @click="buy()"
      class="paypal w-full my-2 inline-block rounded py-3 px-6 no-underline items-center flex justify-center"
    >
      Pay with
      <img
        class="ml-2 h-5"
        src="data:image/svg+xml;base64,"
        alt="paypal"
      />
    </button>
  </div>
</template>

<script>
import postage from "~/data/postage";
import PosterDropdown from "~/components/PosterDropdown";

export default {
  components: {
    PosterDropdown
  },

  data() {
    return {
      error: false,
      currencyCode: "GBP",
      business: "wilsonfamily@ntlworld.com",
      postage,
      shipping: null,
      shippingSelected: null,
      type: null,
      price: null
    };
  },

  computed: {
    totalPrice() {
      return this.shipping + this.price;
    },

    paypalUrl() {
      return `https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=${this.business}&item_name=${this.name}&amount=${this.price}&currency_code=${this.currencyCode}&shipping=${this.shipping}`;
    },

    postageOptions() {
      return this.postage[this.type];
    }
  },

  props: {
    posters: {
      type: Array,
      required: true
    },

    types: {
      type: Object,
      required: true
    },

    item: {
      type: Object,
      required: true
    }
  },

  methods: {
    changeProduct(selection) {
      if (selection) {
        this.price = selection.price;
        this.name = `${this.item.title} ${selection.name}`;
        this.type = selection.type;
      } else {
        this.price = null;
        this.name = null;
        this.type = null;
      }
    },
    updateShipping(value, index) {
      this.shipping = value;
      this.shippingSelected = index;
    },
    formatPrice(value) {
      let val = (value / 1).toFixed(2);
      val = val.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
      return `£${val}`;
    },

    buy() {
      // Validate price
      if (!this.price) {
        this.error = "Please select a product and option";
        return;
      }

      // Validate shipping
      if (!this.shipping) {
        this.error = "Please select a delivery option";
        return;
      }

      window.location.href = this.paypalUrl;
    }
  },

  watch: {
    type() {
      this.shipping = null;
      this.shippingSelected = null;
      this.error = null;
    },
    price() {
      this.error = null;
    },
    shipping() {
      this.error = null;
    }
  }
};
</script>

